...
#### ● Part 1. 전역 공통 및 시스템 아키텍처 (Global Features)
...
##### 6. Section 02 : 'OUR SELECTION' (데이터 기반 동적 렌더링 & 반응형 최적화)
...
##### 8. Section 04 : 'MELT THE VALUES' (가상 공간 매핑 & 위치 동기화 인터랙션)
> * **플렉스 레이아웃의 한계 극복 (가상 플레이스홀더 도입)**: 텍스트와 이미지 영역이 섞인 플렉스 구조에서 마우스 호버 시 이미지가 확장될 때 발생하는 레이아웃 틀어짐 문제를 해결하기 위해, 공간만 차지하는 가상의 빈 영역(`value-item__placeholder`)을 활용한 뼈대 구축
> * **`getBoundingClientRect` 기반 위치 동기화 (`syncMediaPosition`)**: 
>   * 절대 위치(`absolute`)로 띄워진 실제 이미지 영역(`.value-item__media`)이 가상의 플레이스홀더 위치를 정확히 찾아 덮어씌울 수 있도록 부모 요소를 기준으로 한 상대 좌표(`relativeLeft`)를 실시간 계산 및 매핑
>   * 화면 크기 변화(Resize)에 유연하게 대응하여 해상도가 달라져도 이미지 위치가 어긋나지 않도록 정교한 레이아웃 유지
> * **디바이스 맞춤형 인터랙션 최적화**: 
>   * **PC 환경**: 마우스 호버 시 이미지가 부드럽게 전체 너비로 확장(`width: 105%`, `height: 100%`)되며 은은한 어두운 오버레이와 함께 숨겨져 있던 핵심 가치 텍스트(타이틀 및 설명)가 페이드인(`opacity`, `transform`)되도록 구현
>   * **태블릿·모바일 환경(1024px 이하)**: 호버 인터랙션을 무력화하고, 이미지를 고정형 풀 너비 카드 형태로 전환하여 모바일 스크롤 탐색 시 가독성과 시각적 안정성 극대화
> * **리사이즈 최적화 (Debounce 적용)**: 화면 크기 변경 시 과도하게 발생하는 이벤트 부하를 막기 위해 `setTimeout`을 활용한 리사이즈 타이머 제어로 성능 저하 방지

##### 9. Section 06 : 'SHOWROOM' 공간 큐레이션 및 인터랙티브 쇼룸 (Desktop & Mobile Adaptive)
> * **데이터 동적 렌더링 및 모듈화 (`renderShowroomCards`)**: 
>   * 외부 데이터 객체(`showroomData`)의 거실, 주방, 침실 카테고리별 상품 정보를 순회하며 핫스팟 및 상품 카드 마크업을 동적으로 생성하여 코드 재사용성 극대화
> * **PC 환경 : 확장형 쇼룸 및 커스텀 커서 인터랙션**:
>   * **커서 팔로워(`showroomMouseFollower`)**: 쇼룸 섹션 진입 시 기본 커서를 숨기고 마우스 좌표(`e.clientX`, `e.clientY`)를 실시간 추적하는 커서 그룹(`.cursor-follow-group`)을 활성화하여 시각적 몰입감 강화
>   * **그리드 플렉서블 확장(`.showroom__section.active`)**: 세 개의 공간 중 특정 영역 클릭 시 해당 섹션이 유연하게 확장되며(`flex: 0 0 60%`), 내부에 숨겨져 있던 상품 핫스팟(`product-card`)이 페이드인 효과와 함께 노출
> * **스마트 좌표 계산 팝업 알고리즘 (`adjustLinkBoxPosition`)**:
>   * 핫스팟 클릭 시 쇼룸 섹션의 영역 크기(`getBoundingClientRect`)와 중심 좌표를 기준으로 사분면(Quadrant) 판별
>   * 핫스팟이 위치한 사분면 방향에 따라 상품 정보 링크 박스의 노출 위치(`top`, `bottom`, `left`, `right`)를 동적으로 조정하여 박스가 화면 밖으로 잘려 나가는 현상 원천 방지
> * **모바일 환경 : 바텀 시트(Bottom Sheet) 인터페이스 및 웹 접근성 완비**:
...
### ➎ Retrospective
> 이번 프로젝트는 단순히 눈에만 예쁜 포트폴리오용 화면을 넘어, 실제 상용 쇼핑몰 템플릿 제작을 목표로 시작했습니다. 웹쇼핑 특유의 정형화된 틀에서 벗어난 디자인적 요소를 담으면서도, **마우스 호버, 스크롤 업/다운, 화면 리사이즈 등 실제 사용 시 마주할 수 있는 모든 동선과 예외 상황**을 철저히 고려해 코딩하는 데 집중했습니다. 특히 헤더와 슬라이더, 반응형 레이아웃을 구현하며 수많은 예외 상황과 부딪혔고, 그 과정에서 **자바스크립트를 활용한 새로운 해결 접근법**들을 깊이 있게 체득할 수 있었습니다. 모든 코드를 완벽하게 암기하고 있지는 않더라도, 앞으로 실무에서 비슷한 문제를 마주했을 때 **"아, 그때 이런 방식으로 해결했었지!" 하고 떠올리며 찾아볼 수 있는 실무적 문제 해결의 뼈대**를 단단하게 다진 소중한 경험이 되었습니다.

#### 👍 1. Keep (잘한 점)
> * **모든 디바이스 너비와 예외 상황을 고려한 꼼꼼한 완성도**: 개발 과정에서 놓치기 쉬운 다양한 화면 너비와 사용자의 행동 반경(경우의 수)을 꼼꼼히 체크하여, 완성도 높은 레이아웃을 구현해 냈습니다.
> * **실전 트러블슈팅을 통한 레이아웃 버그 해결**: 실제 기기 및 반응형 테스트 과정에서 탑배너와 헤더의 동적 높이 계산으로 인해 발생했던 오프셋 꼬임 및 상단 잘림 현상을, CSS 변수와 `calc` 연산, 미디어 쿼리 분기를 통해 스스로 완벽하게 추적하고 해결했습니다.

#### 🚨 2. Problem (아쉬운 점 / 스스로 느낀 한계)
> * **실무형 심화 메서드 활용 경험 부족**: 자바스크립트의 기초 문법은 숙지하고 있으나, 좌표값 계산이나 복잡한 DOM 제어 등 실무적이고 지엽적인 심화 메서드들이 아직 손에 완전히 익지 않아 최적의 코드로 바로 도출하는 데 다소 아쉬움이 있었습니다.
> * **복잡한 분기 처리에 따른 코드 비효율성**: 다양한 경우의 수를 제어하는 과정에서 조건문이 길어지고 중복이 발생해, 가독성과 확장성 측면에서 코드를 더 스마트하게 압축하지 못한 부분이 내내 아쉬움으로 남았습니다.

#### 🎯 3. Try (개선할 점 및 앞으로의 액션 플랜)
> * **심화 메서드 숙달 및 클린 코드 작성**: 이번 프로젝트를 통해 마주한 복잡한 인터랙션과 좌표 제어 로직들을 복습하며, 불필요한 분기 처리를 줄이고 확장성 있는 코드로 리팩토링하는 연습을 지속하겠습니다.
> * **GSAP & React로의 확장**: 탄탄한 마크업 및 자바스크립트 체급을 바탕으로, GSAP을 활용한 역동적인 인터랙션 구현과 React 기반의 컴포넌트·상태 관리 프로젝트에 차근차근 도전하겠습니다.
